...
from jco.api.models import Address, Account, Jnt, PresaleJnt, Transaction
...
def export_data_to_csv():
    ...
    SELECT * FROM "JNT" ORDER BY id
    SELECT * FROM presale_jnt ORDER BY id
    ...
    jnt_tokens = Jnt.objects.order_by('id')
    presale_jnt = PresaleJnt.objects.order_by('id')
    ...
    zip_file.writestr('jnt_tokens.csv', export_queryset(jnt_tokens).getvalue())
    zip_file.writestr('presale_jnt.csv', export_queryset(presale_jnt).getvalue())
